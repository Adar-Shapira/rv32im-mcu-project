...
--
-- ADAPTED -- one semantic change, in three places, everything else
-- byte-identical (verify with: diff the original against this file from its
-- first '-----' line onward; the diff must show only these):
--   1. line 27 gains a ';' so the port list can continue;
--   2. a new output port RX_BUSY;
--   3. one concurrent assignment driving it, with its comment.
--
