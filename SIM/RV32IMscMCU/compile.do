...
# ---- CDC synchronizer (Phase 4A, Figures 10a/10b) ----
# A leaf module, not yet instantiated by the core - it is compiled here so its
# own self-checking testbench can run, and so a later phase that instantiates it
# finds it already in the library.
vcom -2008 ../../DUT/RV32IMscMCU/SYNC.vhd
vcom -2008 ../../TB/RV32IMscMCU/tb_sync.vhd

# ---- MMIO address decoder (Phase 5A, Figure 5) ----
# Also a leaf for now: Phase 5B instantiates it at the MCU level. Its testbench
# is exhaustive over all 16384 addresses of the §3 data address space, so it runs
# in about 16 us of simulated time and needs no memory images.
