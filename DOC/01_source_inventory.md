...
| --- | --- | --- |
| **Hanan Ribo (lecturer)** | `Auxiliary/Lab 5/Auxilary/RV32I/{DUT,TB,SIM,QUARTUS}/`, the PDFs at that level, `Auxilary/Benchmarks/test1`–`test4/`, `Auxiliary/Benchmark Apps/`, `Auxiliary/Final Project 2026 definition.pdf` | Ten files carry `-- Copyright 20xx Hananya Ribo` on line 2. That header is the only authorship marker in the tree. |
...
| **Hanan Ribo — added 2026-08-25/27** | `Auxiliary/מבנה הצגה ובדיקת פרויקט מסכם.pdf` **and** its `.md` extraction (the project inspection/presentation protocol), `Auxiliary/hanan/` (50 lecture-note files: the full course material — VHDL, architecture, verification, ISMCE validation, the lab syllabus) | Course-official material. **Read the PDF, not the `.md`** — the extraction is garbled (Hebrew stored character-reversed, column order scrambled) and a reconstruction from it got three details wrong, including a technical requirement. The PDF was added 2026-08-27 and `DOC/04` §9 was rewritten against it. `hanan/` is lecture material: `Useful name extensions.md` is the source of this project's `_i`/`_o`/`_w`/`_q`/`_r`/`_v` convention, which every file follows. |
...
>
> All seven are now repaired in `DUT/RV32IMscMCU/` — unconditionally since 2026-08-26, when the
> `G_ISA_REPAIR` switch was retired (Adar, commit `1d16fe2`) after the before/after measurements
> were taken. See the plan file §0.a and Phase 3A.
